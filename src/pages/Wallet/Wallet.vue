<template>
	<view :style="theme" class="page_boxs">
		...
		<!--充值按钮-->
		<view class="xf_cz_btn" v-if="is_login == 1 && tabsidx == 0">
			<view class="Register_btn">
				<view v-if="JinZhiTZ == 0">
					<u-button type="primary" :disabled="isDisabled" :loading="DEPBtnLoading" @click="PanDuaniSGouxuan()">
						<view class="czhi_btnbox">
							<view class="tgyuText">{{$t('DEP')}}<span v-if="wallet" class="tguohu">{{$t('tongguo')}}<text>{{selecValue}}</text></span></view>
							<view class="shijiczje">{{wallet}}<text v-if="wallet">{{YiXuanHuoBiInfo.abbr}}</text></view>
						</view>
					</u-button>
				</view>
				<view v-else-if="JinZhiTZ == 1">
					<view class="xutouzhu">
						...
						<view>{{$t('hbqian')}}{{$t('DEPBtn')}}！<span @click="LianXiKeFu()">{{$t('OrderProgerss.support')}}</span></view>
					</view>
					<u-button type="primary" :disabled="true">{{$t('DEP')}}</u-button>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		onShow() {
			...
			// let makeMoneyTypes = uni.getStorageSync('makeMoneyTypes')
			// if (makeMoneyTypes == 1) {
			// 	that.tabsidx = 0
			// 	uni.setStorageSync("makeMoneyTypes", 0);
			// } else if (makeMoneyTypes == 2) {
			// 	that.tabsidx = 1
			// 	uni.setStorageSync("makeMoneyTypes", 0);
			// }
		},
