<template>
	<view :style="theme" class="page_boxs">
		...
		<match-media :max-width="800" >
			<navbar ref="child" :title="$t('DetailsPage.title')" :isCountTo="false" :is_secondary="false"></navbar>
		</match-media>
	<!-- 	<uni-transition :mode-class="['fade', 'zoom-in']"
			:styles="{'width':'100%','height':'100%','position':'absolute','left':'0'}"
			:show="showOptions"> -->
			<pagesNavTitle ref="" :title="$t('DetailsPage.title')"></pagesNavTitle>

			<view class="page_wrap">

				<!--<view class="type_box_wrap" style="margin-bottom: 20rpx;">
					<view class="type_box" style="margin-right:0;">
						<view class="type_box_item">
							<cus-selects :placeholder="placeholder3" :jiahao="0" :closeSelect.sync='close3' :data="PopularList3" v-model="value3" @change='change3' :valueType="alias3"></cus-selects>
						</view>
					</view>
				</view>-->
				...
				<view class="type_box_wrap">
					<!--<view class="type_box" style="margin-right:10rpx;">
						<view class="type_box_item">
							<cus-selects :placeholder="placeholder2" :jiahao="0" :closeSelect.sync='close2' :data="PopularList2" v-model="value2" @change='change2' :valueType="alias2"></cus-selects>
						</view>
					</view>-->
					...
					<view class="type_box" style="margin-right:0;">
						<view class="type_box_item">
							<cus-selects :placeholder="placeholder3" :jiahao="0" :closeSelect.sync='close3'
								:data="PopularList3" v-model="value3" @change='change3'
								:valueType="alias3"></cus-selects>
						</view>
					</view>

					<view class="type_box" style="margin-left:16rpx;" v-if="islistType == 0">
						<view class="type_box_item">
							<cus-selects :placeholder="placeholder1" :jiahao="0" :closeSelect.sync='close'
								:data="PopularList1" v-model="value" @change='change' :valueType="alias"></cus-selects>
						</view>
					</view>
					<view class="type_box" style="margin-left:10rpx;" v-if="islistType == 1">
						<view class="type_box_item">
							<cus-selects :placeholder="placeholder4" :jiahao="0" :closeSelect.sync='close4'
								:data="PopularList4" v-model="value4" @change='change4'
								:valueType="alias4"></cus-selects>
						</view>
					</view>
							...
							<view class="list_zongguo">
								...
								<view class="right_info">
									<view class="list_item_title">
										...
										<view class="ordersn">
											<!--  :class="item.status == 14||item.status == 15||item.status == 16?'lookDetails':''" @click="[item.status == 14||item.status == 15||item.status == 16?LookDetails(item):null]" -->
									...
									<view class="item_time" style="margin: 10rpx 0 0 0;">
										<view class="time_name">
											...
											<view class="baise" v-if="item.status == 5">{{(item.amount / 100).toFixed(2)}}</view>
											<view class="baise" v-if="item.status == 18">{{(item.amount / 100).toFixed(2)}}</view>
											<view class="baise" v-if="item.status == 10||item.status == 6">{{(item.amount / 100).toFixed(2)}}</view>
											<view class="baise" v-if="item.status == 14||item.status == 15||item.status == 16">{{(item.amount / 100).toFixed(2)}}</view>
											...
											<view class="lvse_box" v-if="item.status == 18"></view><view class="ywcd" v-if="item.status == 18">{{$t('NEWBonusPageA.completed')}}</view>

											<!--<u-icon name="checkmark-circle-fill" color="#45D91E" v-if="item.status == 18"></u-icon>
											
											<u-icon name="checkmark-circle-fill" color="#f3e10c" v-if="item.status == 10||item.status == 6"></u-icon>
											
											<u-icon name="checkmark-circle-fill" color="#f3e10c" v-if="item.status == 5"></u-icon>
											
											<u-icon name="warning-fill" color="#F04B1E" v-if="item.status == 14||item.status == 15||item.status == 16"></u-icon>-->
			...
			<!--<view class="more_btn" v-if="Nomore == 0">
				<u-button type="warning" :loading="btnStatus" @click="LookMore()">Load More</u-button>
			</view>-->
		...
		<Reject-Popup ref="RejectOpen"></Reject-Popup>
		...
		methods: {
			...
			geTxlog() {
				...
				this.$u.post('/withdraw/record', {
					page: this.pageSize, // number 是 页数，首次填1
					status: that.istype, // 查询类型（4.机器审核 5.未支付 6.人工审核 7.二审过通 8.机审通过 9.取消订单 10.准备付款 11.准备退款 12.退款中 13.已退款 14.已驳回 15.订单失败 16.已拒绝 18.成功',）
					dateType: this.isDate //日期类型（0全部1今天2昨天3七天内4三十天内）
				}).then(res => {
					that.page.total = res.data.totalPage * 10
					that.page.pageSize = res.data.pageSize
