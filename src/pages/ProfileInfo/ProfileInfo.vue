<template>
	<view :style="theme" class="page_boxs">
		...
		<match-media :max-width="800" >
			<navbar ref="child" :title="$t('navBarTitle.ProfileInfo')" :isCountTo="false" :is_secondary="false"></navbar>
		</match-media>
		<!-- <uni-transition :mode-class="['fade', 'zoom-in']" :styles="{'width':'100%','height':'100%','position':'absolute','left':'0'}" :show="showOptions"> -->

			<pagesNavTitle ref="" :title="$t('navBarTitle.ProfileInfo')"></pagesNavTitle>
						...
						<view class="ini_input_box">
							<view class="input_item">
								...
								<view class="i_inpout" :class="input_border1 == 1?'input_border':''">
									<u-input v-model="Firstname" @input="usernameInt" @blur="inputBlur1" @focus="inputfocus1" type="text" :border="false" height="75" :custom-style="customStyle" :placeholder-style="placeholderStyle" :placeholder="$t('BiTian')" :clearable="false" />
									<image class="bianjitubiao" src="/static/image/write_icon_148501.png"></image>
								</view>
							</view>
							<!--<view class="input_item">
							<view class="iname">Last name *</view>
							<view class="i_inpout">
								<u-input v-model="Lastname" type="text" :border="false" height="75"
						...
						<view class="ini_input_box">
							...
							<view class="input_item">
								<view class="i_inpout222" @click="Riqishow = true">
									<!--<u-input v-model="yyyy" type="text" :border="false" height="75" :custom-style="customStyle" :placeholder-style="placeholderStyle" placeholder="YYYY" :clearable="false" />-->
							...
							<view class="input_item">
								<view class="i_inpout222" @click="Riqishow = true">
									<!--<u-input v-model="Month" type="text" :border="false" height="75"
									:custom-style="customStyle" :placeholder-style="placeholderStyle"
									placeholder="Month" :clearable="false" />-->
							...
							<view class="input_item">
								<view class="i_inpout222" @click="Riqishow = true">
									<!--<u-input v-model="dd" type="text" :border="false" height="75"
									:custom-style="customStyle" :placeholder-style="placeholderStyle" placeholder="DD"
									:clearable="false" />-->
						...
						<view class="ini_input_box">
							<view class="input_item">
								...
								<view class="i_inpout" :class="input_border2 == 1?'input_border':''">
									<u-input v-model="Email" @input="usernameInt2" @blur="inputBlur2" @focus="inputfocus2" type="text" :border="false" height="75" :custom-style="customStyle" :placeholder-style="placeholderStyle" :placeholder="$t('BiTian')" :clearable="false" :disabled="userInfo.verifyEmail == false?false:true" />

									<view class="Verify" v-if="userInfo.email != '' && userInfo.verifyEmail == false" @click="EmailVerify()">{{$t('phonePopup.verifyBtn')}}</view>
									<image class="bianjitubiao" v-else-if="userInfo.verifyEmail == false" src="/static/image/write_icon_148501.png"></image>
									<image class="bianjitubiao" v-else-if="userInfo.verifyEmail == true" src="/static/image/ok.png"></image>
								</view>
							</view>
						</view>
						<view class="ini_input_box" style="margin: 0;">
							<view class="input_item">
								...
								<view class="quhao_wrap">
									<!--<view style="width: 150rpx;margin-right: 14rpx;" class="i_inpout" :class="input_border3 == 1?'input_border':''">
									<u-input v-model="PhoneCode" @blur="inputBlur3" @focus="inputfocus3" type="text" :border="false" height="75" :custom-style="customStyle" :placeholder-style="placeholderStyle" placeholder="Code" :clearable="false" />
								</view>-->
									<view class="i_inpout i_inpoutbejingse" style="padding: 0 0 0 0;margin-right: 6px;width: auto;">
										<view class="input_item_true">
											<areaCodeSelect :jiahao="1" :placeholder="$t('BiTian')" :closeSelect.sync='close2' :data="selectList2" v-model="value2" @change='change2' :valueType="alias2"></areaCodeSelect>
										</view>
									</view>
									<view class="i_inpout" :class="input_border4 == 1?'input_border':''">
										<u-input v-model="Phone" @input="usernameInt3" @blur="inputBlur4" @focus="inputfocus4" type="number" :border="false" height="75" :custom-style="customStylePhone" :placeholder-style="placeholderStyle" :placeholder="$t('BiTian')" :clearable="false" :disabled="false" /><!--userInfo.verifyPhone == false?false:true-->
										<view class="Verify" v-if="userInfo.phoneNumber != '' && userInfo.verifyPhone == false" @click="phoneNumberVerify()">{{$t('phonePopup.verifyBtn')}}</view>
										<image class="bianjitubiao" v-else-if="userInfo.verifyPhone == false" src="/static/image/write_icon_148501.png"></image>
										<image class="bianjitubiao" v-else-if="userInfo.verifyPhone == true" src="/static/image/ok.png"></image>
									</view>
								</view>
			...
			getuserinfo() {
				...
				that.$u.post('/user/get/info', {}).then(res => {
					...
					if (match) {
						...
						that.value2 = this.AreaCode
						that.PhoneCode = this.AreaCode
					}
		...
		.myinfo_inputzu {
			...
			.ini_input_box {
				...
				.input_item {
					...
					.i_inpoutbejingse {
						//background-color: #1c2838 !important;
						//border: 1px solid #1c2838 !important;
						border-radius: 10px;
					}
