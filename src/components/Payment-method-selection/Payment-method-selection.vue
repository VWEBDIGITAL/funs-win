				...
				<view class="select_modal_con show-more" v-if="show" @touchmove.stop.prevent="() => {}">
					<scroll-view scroll-y="true" class="select_modal select_modal_scroll" :style="{height:'auto'}">
						...
						<view class="select_content" ref="select_content">
							<view v-for="(item, index) in showDatas" :key="index" class="select_content_li" :class="{'selected': CRvalue == item.index}" @click="change(item,index)">
...
<script>
	export default {
		...
		props: {
			...
			defaultIcon: {
				type: String,
				default: '/static/logo/1_logo.png'
			},
		},
		data() {
			return {
		...
		methods: {
			...
			change(item,index) {
				...
				this.$emit('LowchangeTwo', item,index)
