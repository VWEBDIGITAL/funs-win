<template>
	<view :style="theme">

		<u-popup v-model="RegiserShow" width="650" mode="center" close-icon-size="30" border-radius="14" :mask-close-able="false" :closeable="true">
			<view class="reg_title" @click="RegiserShow = false">
				<view>{{$t('ManageAccountPage.AddAccound')}}</view>
			</view>
			<view class="input_box">
				<view class="input_item">
					<view class="input_item_title">{{$t('XuanZeQianBao')}} *</view>
					<view class="input_item_true_cus" style="padding: 0;">
						<cus-selects :closeSelect.sync='close' :data="selectList" v-model="value" @change='change' :valueType="alias" @Lowchange="Lowchange"></cus-selects>
					</view>
				</view>

				<view class="input_item">
					<view class="input_item_title">{{$t('QianBaoDiZhi')}} *</view>
					<view class="input_item_true" :class="input_border1 == 1?'input_border':''">
						<u-input v-model="wallet" @input="input1" @blur="inputBlur1" @focus="inputfocus1" type="text" :border="false" :custom-style="customStyle" :placeholder-style="placeholderStyle" placeholder="" :clearable="false" />
					</view>
				</view>

				<view class="input_item">
					<view class="input_item_title">{{$t('NewlyAdded.Realname')}} *</view>
					<view class="input_item_true" :class="input_border2 == 1?'input_border':''">
						<u-input v-model="beizhu" @input="input2" @blur="inputBlur2" @focus="inputfocus2" type="text" :border="false" :custom-style="customStyle" :placeholder-style="placeholderStyle" placeholder="" :clearable="false" />
					</view>
				</view>

				<view class="Agreement">
					<view>
						<u-icon name="info-circle-fill"></u-icon> {{$t('NewlyAdded.Addhint')}}
					</view>
				</view>
				<view class="Register_btn">
					<u-button type="primary" :disabled="isDisabledTwo" @click="Baocun()">{{$t('NewlyAdded.Savebutton')}}</u-button>
				</view>
			</view>
		</u-popup>

	</view>
</template>

<script>
	import BankData from "@/utils/bankList.js";
	import cusSelects from '@/components/cus-selects-fan/cus-selects-fan.vue'
	export default {
		name: "Addac-purse",
		props: {

		},
		components: {
			cusSelects
		},
		data() {
			return {
				input_border1: 0,
				input_border2: 0,
				wallet: '',
				beizhu: '',
				
				isDisabledTwo: true,
				RegiserShow: false,
				placeholderStyle: "color: #8e99a2;fontSize: 28rpx;fontWeight: initial;",
				customStyle: {
					fontWeight: "bold"
				},


				value: '128',
				selectList: [],
				alias: { //别名来代指一下
					label: 'name',
					value: 'id'
				},
				close: false,
				
				
				isbnakCode: '128',
				isbnakName: 'GOPAY钱包'


			};
		},
		watch: {

		},
		computed: {

		},
		created() {

		},
		mounted() {

		},
		methods: {
			
			
			input1(e) {
				if (!e) {
					this.isDisabledTwo = true
				} else {
					this.isDisabledTwo = false
				}
			},
			inputBlur1(e) {
				this.input_border1 = 0
				
				// //只能输入数字
				// const inputType1 = /[^\d]/g
				 
				// //只能输入字母
				// const inputType2 = /[^a-zA-Z]/g  
				
				// //只能输入数字和字母
				// const inputType3 = /[\W]/g
				
				// //只能输入小写字母
				// const inputType4 = /[^a-z]/g
				
				// //只能输入大写字母
				// const inputType5 = /[^A-Z]/g
				
				// //只能输入数字和字母和下划线
				// const inputType6 = /[^\w_]/g //下划线也可以改成%
				
				// //只能输入中文
				// const inputType7 = /[^\u4E00-\u9FA5]/g
				
				// //只能输入数字和小数点
				// const inputType7 = /[^\d.]/g
				
				let screeningStr = /[\W]/g; //想禁止什么类型，在这里替换正则就可以了
				if(screeningStr.test(e)){
					this.$u.toast(this.$t('DiZhiWuXiao'));
					this.$nextTick(() => {
						this.wallet = '';
					});
					this.isDisabledTwo = true
				} else {
					this.$nextTick(() => {
						this.wallet = e;
					});
				}
			},
			inputfocus1() {
				this.input_border1 = 1
			},
			
			
			
			
			input2(e) {
				if (!e) {
					this.isDisabledTwo = true
				} else {
					this.isDisabledTwo = false
				}
			},
			inputBlur2(e) {
				this.input_border2 = 0
				let screeningStr = /^[\a-\z\A-\Z0-9\u4e00-\u9fe5]+$/;//只能输入中文数字和英文
				if(screeningStr.test(e)){
					this.$nextTick(() => {
						this.beizhu = e;
					});
				} else {
					this.$u.toast(this.$t('nameWuXiao'));
					this.$nextTick(() => {
						this.beizhu = '';
					});
					this.isDisabledTwo = true
				}
			},
			inputfocus2() {
				this.input_border2 = 1
			},
			
			
			
			
			
			
			
			
			
			
			
			
			change(e) {
				////console.log(e) //这里返回的是value值
			},
			Lowchange(e){
				//console.log('选择的钱包',e) //这里返回的是value值
				this.isbnakCode = e.id
				this.isbnakName = e.name
			},
			openPopup(data) {
				//console.log('',data)
				
				//设置默认选中的钱包
				this.value = data[0].id
				this.isbnakCode = data[0].id
				this.isbnakName = data[0].name
				
				this.selectList = data
				this.RegiserShow = true
			},
			closewindows() {
				this.RegiserShow = false
			},
			Baocun() {
				let that = this

				let serialNo = uni.getStorageSync('thisUuid')
				let machineName = uni.getStorageSync('machineName')
				let ua = uni.getStorageSync('thisUa')

				if (this.wallet == "") {
					this.$u.toast(this.$t('NewlyAdded.WithdralAddress'));
				} else if (this.beizhu == "") {
					this.$u.toast(this.$t('ToastPrompt.Toast16'));
				} else {
					that.$u.post('/withdraw/bind', {
						method: 10, // number 是 绑定渠道类型
						
						phoneNum: '', // String 否 手机号码 格式：(+00)186123
						email: '', // String 否 电子邮件
						
						deviceId: serialNo,
						deviceType: machineName || ua,
						
						
						account: that.wallet, //钱包地址，或账户名，卡号
						realName: that.beizhu,
						
						bankCode: String(that.isbnakCode),
						bankName: that.isbnakName,
						
						
					}).then(res => {
						
						that.wallet = ""
						that.beizhu = ""
						that.isbnakCode = "2"
						that.isbnakName = "GOPAY钱包"
						
						that.$emit('zhidiBottom')
						that.$emit('GetAcList')
						that.RegiserShow = false
					})
				}
			}
		}
	}
</script>

<style lang="scss">
	.to_SignIn {
		background-color: $all-secondary-color;
		text-align: center;
		height: 83rpx;
		line-height: 83rpx;
		color: $font-all-color;
		font-size: 24rpx;
	}

	.input_box {
		padding: 30rpx;

		.input_item {
			margin-bottom: 30rpx;

			/deep/ .select_modal_scroll {
				height: auto !important;
			}

			.input_item_title {
				color: $font-all-color;
				margin-bottom: 10rpx;
				font-size: 28rpx;
			}

			.input_item_true {
				background-color: $all-secondary-color;
				border: 1px solid $input-border-color;
				border-radius: 10px;
				padding: 4px 16rpx;
			}
		}

		.Agreement {
			margin-top: 20rpx;
			font-size: 24rpx;
			color: $font-all-color;

			.u-icon {
				margin-right: 10rpx;
			}
		}

		.Register_btn {
			width: 100%;
			margin: 40rpx auto 0;
		}
	}

	.reg_title {
		padding: 30rpx 30rpx;
		display: flex;
		align-items: center;
		justify-content: space-between;

		view {
			font-size: 30rpx;
			color: $all-secondary-text-color;
			//font-weight: bold;
		}
	}
</style>