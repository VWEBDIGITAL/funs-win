			...
			<scroll-view scroll-x class="u-scroll-view" :scroll-left="scrollLeft" scroll-with-animation>
				<view class="u-scroll-box" :class="{'u-tabs-scorll-flex': !isScroll}">
					<view class="u-tab-item u-line-1" :id="'u-tab-item-' + index" v-for="(item, index) in list" :key="index" @tap="clickTab(index)" :style="[tabItemStyle(index)]">
						...
						<view class="tabs_icon" :class="[index == currentIndex?'xuanzhong':'weixuanzhong']">
							<view class="left_icon_box">
								<!-- <image class="left_icon" :class="current == index?'zhihui':''" :src="item.iconImage" mode="widthFix"></image> -->
								...
								<view v-if="item.typeName && item.typeName=='全部'">{{$t('TransactionPage.TraOption0')}}</view>
								<view v-else>{{item.typeName}}</view>
								<view class="game_num" v-if="current == index">{{item.lists.length}}</view>
...
<script>
	...
	export default {
			...
			tabItemStyle() {
				return (index) => {
					let style = {
						...
						//'backgroundImage': 'linear-gradient(to right, #1d2532, #1d2532)'
					};
					// 字体加粗
					if (index == this.currentIndex && this.bold) style.fontWeight = 'bold';
					if (index == this.currentIndex) {
						//style.color = this.activeColor;
						//style.backgroundImage = "linear-gradient(to right, #548dff, #0c5eff)";//添加选中的背景色
		...
		methods: {
			QiuHe(item){
				//console.log('大幅度',item)
				let zong = item.vendors.length
...
<style lang="scss" scoped>
	@import "@/uview-ui/libs/scss/style.components.scss";
	...
	.weixuanzhong {
		background-image: linear-gradient(to right, $bonus-tabs-inactivebg, $bonus-tabs-inactivebg) !important;
		color: $bonus-tabs-inactivecolor !important;
	}
