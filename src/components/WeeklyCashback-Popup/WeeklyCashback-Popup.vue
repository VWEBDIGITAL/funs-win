	...
	<view :style="theme">
		...
		<u-popup v-model="AllLevelShow" width="650" mode="center" close-icon-size="30" border-radius="14"
			...
			<view class="reg_title" @click="AllLevelShow = false">
				<view>{{$t('WeeklyCashBackPopup.title')}}</view>
			</view>
			<view class="input_box">

				<view class="popTitle">{{$t('WeeklyCashBackPopup.prompt')}}</view>
				<view class="line"></view>
				<view class="poptitle2">
					<view>{{$t('WeeklyCashBackPopup.Calculationcycle')}}:</view>
					<view>{{$t('WeeklyCashBackPopup.Calculation')}}</view>
				</view>
				<view class="line"></view>
				<view class="poptitle2">
					<view>{{$t('WeeklyCashBackPopup.Claimtime')}}</view>
					<view>
						<u-count-down :timestamp="Datas.countdown" bg-color="" color="#aebac8" separator-color="#aebac8"
							font-size="28"></u-count-down>
					</view>
				</view>
				<view class="fubiaoti_box">
					<view><u-icon name="info-circle-fill" size="34" color="#4b9e5f"></u-icon></view>
					<view>{{$t('WeeklyCashBackPopup.TIP')}}</view>
				</view>


				<view class="tab_list_wrap">
					<view class="tab_list_title">
						<view>{{$t('WeeklyCashBackPopup.WeeklyLoss')}}</view>
						<view>{{$t('WeeklyCashBackPopup.CashBack')}}</view>
						<view>{{$t('WeeklyCashBackPopup.MaxBonus')}}</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			Getkuisuan() {
				//
				let that = this
				that.$u.post('/weekly/cashback/cfg', {

				}).then(res => {
					//console.log('周亏损配置', res.data)
