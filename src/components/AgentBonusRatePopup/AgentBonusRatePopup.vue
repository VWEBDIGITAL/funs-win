	...
	<view :style="theme">
		...
		<u-popup v-model="AgentBonusRatePopupShow" width="650" height="90%" mode="center" close-icon-size="30" border-radius="24" :mask-close-able="false" :closeable="true">
			<view class="reg_title" @click="AgentBonusRatePopupShow = false">
				<view>{{$t('CommissionRatePopup.title')}}</view>
			</view>
			<view class="input_box">
				...
				<view class="tab_list_wrap">
					<view class="tab_list_title">
						...
						<view>{{$t('CommissionRatePopup.ValidBets')}}</view>
						<view>{{$t('CommissionRatePopup.Commission')}}</view>
					</view>
							...
							<view class="tabl_list_box_item_box" :class="[statLevel == index?'mylevel':'']">
								<view class="level_box">
									<view class="level_box_item">
										<view><u-icon v-if="statLevel == index" name="checkmark-circle-fill" size="28" color="#ffffff"></u-icon></view>
										<view>{{item.level}}</view>
									</view>
								</view>
								<view>{{item.validBet/100}}</view>
								<view>{{item.rebate}}/10K</view>
							</view>
						</view>
					</view>
				</view>
				
				<view class="jieshao_box">
					<view>{{$t('CommissionPopup.title1')}}</view>
					<view>{{$t('CommissionPopup.title2')}}</view>
				</view>
...
<script>
	export default {
		...
		methods: {
			...
			getData(){
				let that = this
				that.$u.post('/promotion/rebate/cfg', {
