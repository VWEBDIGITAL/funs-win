<template>
	<view class="u-char-box">
		<view class="u-char-flex">
			<input :disabled="disabledKeyboard" :value="valueModel" type="number" :focus="focus" @blur="isblur" @focus="isfocus" :maxlength="maxlength" class="u-input" @input="getVal"/>
			<view v-for="(item, index) in loopCharArr" :key="index">
				<view :class="[breathe && charArrLength == index && shifujujiao ? 'u-breathe' : '', 'u-char-item', charArrLength === index && mode == 'box' ? 'u-box-active' : '', mode === 'box' ? 'u-box' : '']" :style="{
					fontWeight: bold ? 'bold' : 'normal',
					fontSize: fontSize + 'rpx',
					width: width + 'rpx',
					height: width + 'rpx',
					color: inactiveColor,
					borderColor: charArrLength === index && mode == 'box' && shifujujiao ? activeColor : inactiveColor
				}">
					<view class="u-placeholder-line" :style="{
							display: charArrLength === index && shifujujiao ? 'block' : 'none',
...
<script>
	...
	export default {
		...
		methods: {
			isfocus(e){
				////console.log('聚焦了',e)
				this.shifujujiao = true
			},
			isblur(e){
				////console.log('离开了',e)
				this.shifujujiao = false
			},
			getVal(e) {
				let {
					value
