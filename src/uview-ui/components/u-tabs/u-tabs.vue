				...
				<view class="u-scroll-box" :id="id" :class="{'u-tabs-scroll-flex': !isScroll}">
					<view class="u-tab-item u-line-1" :id="'u-tab-item-' + index" v-for="(item, index) in list" :key="index" @tap="clickTab(index)" :style="[tabItemStyle(index)]">
						...
						<view class="tabs_icon">
							...
							<view :class="[current == index?'bianbai':'morenc']" v-if="item.name && item.name=='Live'">{{$t('LiveCasino')}}</view>
							<view :class="[current == index?'bianbai':'morenc']" v-if="item.name && item.name=='Sports'">{{$t('Sports')}}</view>
							<view :class="[current == index?'bianbai':'morenc']" v-if="item.name && item.name=='Cards'">{{$t('Cards')}}</view>
							<view :class="[current == index?'bianbai':'morenc']" v-if="item.name && item.name=='Mini'">{{$t('MiniGames')}}</view>
							<view :class="[current == index?'bianbai':'morenc']" v-if="item.name && item.name=='Bingo'">{{$t('ADDnew.Lottery')}}</view>
...
<script>
	...
	export default {
		...
		created() {
			this.domains = uni.getStorageSync("domains")
		},
		watch: {
			...
			tabItemStyle() {
				return (index) => {
					let style = {
						...
						width: 'auto',//this.$u.addUnit(this.itemWidth),
						'border-radius': '8px'
					};
					// 字体加粗
