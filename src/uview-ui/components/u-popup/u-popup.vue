<template>
	<view :style="theme">
		<view v-if="visibleSync" :style="[customStyle, { zIndex: uZindex - 1 }]" class="u-drawer" hover-stop-propagation>
			...
			<view class="u-drawer-content" @tap="modeCenterClose(mode)" :class="[ safeAreaInsetBottom ? 'safe-area-inset-bottom' : '', 'u-drawer-' + mode, showDrawer ? 'u-drawer-content-visible' : '', zoom && mode == 'center' ? 'u-animation-zoom' : '' ]" @touchmove.stop.prevent @tap.stop.prevent :style="[style]">
				
				<view class="u-mode-center-box" @tap.stop.prevent @touchmove.stop.prevent v-if="mode == 'center'" :style="[centerStyle]">
					<u-icon @click="close" v-if="closeable" class="u-close" :class="['u-close--' + closeIconPos]" :name="closeIcon"></u-icon>
				...
				<view @tap="close" class="u-close" :class="['u-close--' + closeIconPos]">
					<u-icon v-if="mode != 'center' && closeable" :name="closeIcon"></u-icon>
				</view>
	...
	export default {
		...
		computed: {
			...
			centerStyle() {
				...
				style.maxHeight = this.height ? this.getUnitValue(this.height) : 'auto';
				style.zIndex = this.uZindex;
...
<style scoped lang="scss">
	...
	/deep/ .u-icon {
		...
		color: $pupUp-title-closeIcon-color !important;
	}
