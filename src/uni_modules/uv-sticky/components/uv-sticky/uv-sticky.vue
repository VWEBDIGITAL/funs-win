<template>
	<view class="uv-sticky" :id="elId" :style="[theme, style]" >
		...
		methods: {
			...
			getStickyTop() {
				...
				that.$nextTick(() => {
					...
					// #ifdef APP-PLUS
					that.stickyTop = 58 + uni.getSystemInfoSync().statusBarHeight
					// #endif
					
					// #ifdef MP-WEIXIN
					that.stickyTop = 44 + uni.getSystemInfoSync().statusBarHeight
					// #endif
					
					// #ifdef H5
					//that.stickyTop = 0
					that.stickyTop = that.$uv.getPx(that.offsetTop) + that.$uv.getPx(that.customNavHeight)
					// #endif
...
<style lang="scss" scoped>
	@import '@/uni_modules/uv-ui-tools/libs/scss/components.scss';
	.uv-sticky {
		/* #ifdef APP-VUE || MP-WEIXIN */
		// 此处默认写sticky属性，是为了给微信和APP通过uni.createSelectorQuery查询是否支持css sticky使用
		position: sticky;
		/* #endif */
		background-color: $xiding-bg-color !important;
	}
</style>
